...
deque_t* construct()
{
    deque_t* newqueue = (deque_t*) malloc(sizeof(deque_t));
    newqueue->front = NULL;
    newqueue->rear = NULL;
    newqueue->size = 0;
    return newqueue;
...
//Retorna o valor do fim da lista
int rear (deque_t *deque)
{
...
void enqueue_rear(deque_t *deque, int value)
{
    ...
    deque->rear->next = newtail;
    newtail->prev = deque->rear;
    deque->rear->next = newtail;
    deque->rear = newtail;
}
...
void enqueue_front(deque_t *deque, int value)
{
    node_t* newfront = node_new(value);
    deque->size++;
...
void dequeue_rear (deque_t *deque)
{
    if(!empty(deque))
    {
        deque->rear->prev->next = deque->rear->next;
...
void dequeue_front(deque_t *deque)
{
    if(!empty(deque))
    {
        deque->front->next->prev = deque->front->prev;
